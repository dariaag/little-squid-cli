...
use super::{block_fetcher::BlockFetcher, transaction_fetcher::TransactionFetcher};
#[derive(Debug, Clone)]
...
pub async fn block_loop<T: DataFetcher>(
    mut start_block: u64,
    end_block: u64,
    fields: Vec<String>,
    ...
    loop {
        ...
        let data_fetcher: BlockFetcher = DataFetcher::new();
        let (block_chunk, next_block) = data_fetcher
            .fetch_data(start_block, &fields, client, None)
            .await
            .unwrap();
